...
<form method="get"  action="admin_total_qna_list">
    <table id="notice"> 
        <colgroup>
            <col style="width: 7%">
            <col style="width: 13%">
		 ...
		 <tr>
            ...
            <td style="padding: 5px;">
                ...
                <input type="checkbox" id="q_answer+${q.q_no }">
					...
					<input type="button" value="답변" onclick="location= 'total_qna_cont?no=${q.q_no}&page=${page}&state=reply';" />
   					<input type="button" value="수정" onclick="location= 'total_qna_cont?no=${q.q_no}&page=${page}&state=edit';" />
   					<input type="button" value="삭제" onclick="location= 'total_qna_cont?no=${q.q_no}&page=${page}&state=del';" /></div>
						</p>
					</div>
            </td>
            ...
            <td align="center">
            	<input style="margin: 5px 0;" type="button" value="답변" onclick="location='admin_total_qna_cont?no=${q.q_no}&page=${page}&state=reply';" /> 
            	<input type="button" value="수정" onclick="location='admin_total_qna_cont?no=${q.q_no}&page=${page}&state=edit';"/>
				<input style="margin: 5px 0;" type="button" value="삭제" onclick="if(confirm('정말로 삭제할까요?') == true)
													{location='admin_total_qna_del?no=${q.q_no}&page=${page}';}else{ return ;}" />  
            </td>
       	 </tr>
        ...
        <%--페이징 즉 쪽나누기 추가 --%>
			<div>
				...
				<c:if test="${(empty find_field) && (empty find_name)}">
					...
					<c:if test="${page>1}"><a href="admin_total_qna_list?page=${page-1}">[이전]</a>&nbsp;</c:if>

					<%--현재 쪽번호 출력--%>
					<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
						<c:if test="${a == page}"><${a}></c:if>	<%--현재 페이지가 선택되었다면--%>
						<c:if test="${a != page}"><a href="admin_total_qna_list?page=${a}">[${a}]</a>&nbsp;</c:if> 	<%--현재 페이지가 선택되지 않았다면 --%>
					</c:forEach>

					<c:if test="${page >= maxpage}">[다음]</c:if>
					<c:if test="${page<maxpage}"><a href="admin_total_qna_list?page=${page+1}">[다음]</a></c:if>
				</c:if>
				...
				<c:if test="${(!empty find_field) || (!empty find_name)}">
					<c:if test="${page<=1}">[이전]&nbsp;</c:if>
					<c:if test="${page>1}">
						<a href="admin_total_qna_list?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
    				</c:if>
					...
					<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
						<c:if test="${a == page}"><${a}></c:if>	<%--현재 페이지가 선택되었다면--%>
						<c:if test="${a != page}">				
							<a href="admin_total_qna_list?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
     					</c:if><%--현재 페이지가 선택되지 않았다면 --%>
					</c:forEach>

					<c:if test="${page >= maxpage}">[다음]</c:if>
					<c:if test="${page<maxpage}">
						<a href="admin_total_qna_list?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
					</c:if>
				</c:if>
			</div>
    </div>
