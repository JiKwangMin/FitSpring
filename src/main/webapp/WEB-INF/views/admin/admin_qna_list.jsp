...
<form method="get"  action="admin_qna_list">
    <table id="notice"> 
        <colgroup>
            <col style="width: 7%">
            <col style="width: 13%">
...
<!-- 루프 -->
		 ...
		 <tr>
            ...
            <td align="center">
            	<input style="margin: 5px 0;" type="button" value="답변" onclick="location='admin_qna_cont?no=${q.q_no}&page=${page}&state=reply';" /> 
            	<input type="button" value="수정" onclick="location='admin_qna_cont?no=${q.q_no}&page=${page}&state=edit';"/>
				<input style="margin: 5px 0;" type="button" value="삭제" onclick="if(confirm('정말로 삭제할까요?') == true)
													{location='admin_qna_del?no=${q.q_no}&page=${page}';}else{ return ;}" />  
            </td>
       	 </tr>
